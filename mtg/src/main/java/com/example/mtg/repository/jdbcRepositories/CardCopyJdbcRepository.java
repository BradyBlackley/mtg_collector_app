...
public class CardCopyJdbcRepository implements CardCopyRepository {
    ...
    public List<CardCopy> findAllByUser(String userId) {
        final String sql = "select * from card_copy where user_id = ?;";
    ...
    @Override
    public List<CardCopy> findAllByCardId(String cardId) {
        final String sql = "select * from card_copy where card_id = ?;";
        List<CardCopy> cardCopies = jdbcTemplate.query(sql, new CardCopyMapper(), cardId);
        for (CardCopy cardCopy : cardCopies) {
            if(cardCopy != null) {
                addUser(cardCopy);
                addCard(cardCopy);
    ...
    public CardCopy findByCardCopyId(int cardCopyId) {
        final String sql = "select * from card_copy where card_copy_id = ?;";
    ...
    public CardCopy add(CardCopy cardCopy) {
        final String sql = "insert into card_copy (card_id, user_id) values (?,?);";
        int rowsAffected = jdbcTemplate.update(sql, cardCopy.getCard().getCardId(), cardCopy.getUser().getUserId());
        if (rowsAffected <= 0) {
            return null;
        }
        addUser(cardCopy);
    ...
    public boolean update(CardCopy cardCopy) {
        final String sql = "update card_copy set card_id = ?, user_id = ? where card_copy_id = ?;";
    ...
    public boolean delete(int cardCopyId) {
        final String sql = "delete from card_copy where card_copy_id = ?;";
